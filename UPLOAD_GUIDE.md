...
#### 步骤1：准备修改
1. **更新配置文件**：修改 `assets/config/` 目录下的相应配置文件
2. **更新图片**：上传或替换 `assets/images/` 目录下的图片

#### 步骤2：构建项目
有两种方式构建项目：

**方式A：使用build.bat脚本（推荐）**
1. 双击运行 `build.bat` 文件
2. 脚本会自动：
   - 创建必要的目录结构
   - 清除旧的图片文件
...
#### 步骤4：等待部署
- Cloudflare Pages会自动检测到Git仓库的更改
- 自动开始构建和部署过程
- 部署通常需要1-2分钟完成
...
```
┌─────────────────┐     ┌─────────────────┐     ┌─────────────────┐
│ 修改配置文件/图片 │ ──→ │ 运行build.bat   │ ──→ │ 运行push.ps1    │
└─────────────────┘     └─────────────────┘     └─────────────────┘
                                                           │
                                                           ▼
                                            ┌──────────────────────────┐
